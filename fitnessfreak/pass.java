...
public class pass extends JFrame implements ActionListener{
    ...
    public pass() {
        loginLayout12 customLayout = new loginLayout12();
		...
		chpass2.addKeyListener(new KeyAdapter()
		{
			...
			{  Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
				...
				ResultSet set=stat.executeQuery("Select * from login");
				set.next();
			...
			if(flag==true)
			{
				if(chuname2.getText().equals("")|| chpass2.getPassword().length==0)
				{
						...
						JOptionPane.showMessageDialog(pass.this,"<html><font size=4 color=red>Enter New User Name and Password</font></html>","change Password",JOptionPane.ERROR_MESSAGE,error);
				}
				else
				{
				...
				try
				{
					...
					PreparedStatement ps=conn.prepareStatement("UPDATE login set u_name=?,pass=? where u_name=?");
					...
					JOptionPane.showMessageDialog(pass.this,"<html><font size=4 color=green>Password Is Changed</font></html> \n\t\t Use New password","Change Password",JOptionPane.INFORMATION_MESSAGE,info);
					dispose();
				}
				catch(Exception e2)
				{
			...
			{
				 Icon error=new ImageIcon("error.png");
				JOptionPane.showMessageDialog(pass.this,"<html><font size=4 color=red>Invalid Password </font></html> \n\t\t Please enter valid password","Login",JOptionPane.ERROR_MESSAGE,error);
			}
			}
			}
		});
...
class loginLayout12 implements LayoutManager {

    public loginLayout12() {
    }
