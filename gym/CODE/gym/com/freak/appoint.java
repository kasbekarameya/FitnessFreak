...
public class appoint extends JFrame implements ActionListener,ItemListener{
    ...
    public appoint() {
		...
		ad=new ImageIcon("gym/add.png");
       	sav=new ImageIcon("gym/save.png");
       	de=new ImageIcon("gym/dust.PNG");
       	ca=new ImageIcon("gym/close.png");
 ...
 for(int i = numrows - 1; i >=0; i--)   
 {
			...
			try
				{
					...
					DataInputStream sin=new DataInputStream(new FileInputStream("gym/App.dat"));
	...
	PreparedStatement pm=con.prepareStatement("insert into appointment(appid,apptype,appdate,memname,empname,hrs,min,am,apptime,mid)values(?,?,?,?,?,?,?,?,?,?)");
		...
		cmbdate.setValue(new java.util.Date());
					...
					try{
			DataInputStream dt=new DataInputStream(new FileInputStream("gym/App.dat"));
			...
			int len=coursename.length();
							...
							DataOutputStream out=new DataOutputStream(new FileOutputStream("gym/App.dat"));
							out.writeUTF(temp2);
							dt.close();
